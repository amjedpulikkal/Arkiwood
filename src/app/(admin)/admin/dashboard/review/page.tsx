...
"use client";
import { toast } from "sonner";
import React, { useEffect, useState, ChangeEvent, MouseEvent, FC } from "react";
import {
  Star,
  User,
  MessageSquare,
...
type ReviewType = "project" | "service";

interface ReviewFormProps {
  isOpen: boolean;
  onClose: () => void;
  type?: ReviewType;
  itemData?: { title?: string } | null;
...
const ReviewForm: FC<ReviewFormProps> = ({
  ...
  const [formData, setFormData] = useState<FormDataShape>({
    ...
    projectPhase: type === "project" ? "completed" : "",
    company: "",
    professionalism: type === "service" ? 5 : undefined,
  });
  const [hoveredRating, setHoveredRating] = useState(0);
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const handleCopy = () => {
    const url = `https://arkiwooduae.com/testimonial/${formData.dynamic_link}`;
    navigator.clipboard
      .writeText(url)
      .then(() => {
  ...
  const handleInputChange = <K extends keyof FormDataShape>(
    field: K,
  ...
  const handleDynamicLink = (
    ...
    setIsDynamicLinkEnabled(checked);
    if (checked) {
      const id = nanoid(10);
      setFormData((prev) => ({ ...prev, dynamic_link: id }));
    } else {
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await fetch(
        "/api/services/getServices?serviceId=true"
      );
      const { data } = await res.json();
      setServices(data || []);
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      {/* ...UI markup unchanged, types inferred by TSX... */}
    </div>
  );
};

// Demo component to show usage
const ReviewFormDemo: FC = () => {
  const [showProjectReview, setShowProjectReview] = useState(false);
